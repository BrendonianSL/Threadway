...
//Router Responsible For Grabbing Existing Threads.
router.get('/', async (req, res, next) => {
    try {
        ...
        const { data, error } = await req.supabase.from('threads').select('*').eq('user_id', req.user.id);
        ...
        data.forEach(thread => {
            // Update thread.link after replacing the encoded slashes
            thread.link = thread.link.replace(/&#x2F;/g, '/');
        });
